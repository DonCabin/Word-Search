...
void busca(char matriz[16][60], char palavra[10]){
	char test[8];
	for(int i =0; i< 16; i++){
		for(int j = 0; j<60; j++){
			if(matriz[i][j] == palavra[0]){
				//printf("%c",matriz[i][j] );
				//printf("%c\n",palavra[0] );
				for(int direita=1; direita<strlen(palavra);direita++){
					if((matriz[i][j+direita] == palavra[direita])){
						if(strlen(palavra)==direita+1){
							printf("palavra = %s\n posicao_inicial = [%d][%d]\n posica_final  = [%d][%d]\n",palavra, i,j,i,j+direita+1);
							
						}
					}else{continue;}
				}
				for(int esquerda=1;esquerda<strlen(palavra);esquerda++){
					if((matriz[i][j-esquerda] == palavra[esquerda])&&(j-esquerda>-1)){
						if(strlen(palavra)==esquerda+1){
							printf("palavra = %s\n posicao_inicial = [%d][%d]\n posica_final  = [%d][%d]\n",palavra, i,j,i,j-esquerda+1);
						}
					}else{continue;}
				}
				for(int cima=1;cima<strlen(palavra);cima++){
					if((matriz[i-cima][j] == palavra[cima])&&(i-cima>-1)){
						if(strlen(palavra)==cima+1){
							printf("palavra = %s\n posicao_inicial = [%d][%d]\n posica_final  = [%d][%d]\n",palavra, i,j,i-cima+1,j);
						}
					}else{continue;}
				}
				for(int baixo=1;baixo<strlen(palavra);baixo++){
					if((matriz[i+baixo][j] == palavra[baixo])&&(i+baixo<60)){
						if(strlen(palavra)==baixo+1){
							printf("palavra = %s\n posicao_inicial = [%d][%d]\n posica_final  = [%d][%d]\n",palavra, i,j,i+baixo+1,j);
						}
					}else{continue;}
